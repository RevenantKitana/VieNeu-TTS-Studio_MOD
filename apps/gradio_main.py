...
<div class="header-box">
    ...
    <div class="header-links">
        ...
        <a href="https://github.com/pnnbao97/VieNeu-TTS" target="_blank" class="header-link-pill" title="Mã nguồn GitHub">
            <span>🐙</span> GitHub
        </a>
        <a href="https://www.facebook.com/pnnbao97" target="_blank" class="header-link-pill" title="Tác giả gốc: Phạm Nguyễn Ngọc Bảo">
            <span>👤</span> Ngọc Bảo
        </a>
        <a href="https://discord.gg/yJt8kzjzWZ" target="_blank" class="header-link-pill" title="Cộng đồng Discord VieNeu">
            <span>💬</span> Discord
        </a>
    </div>
</div>
                ...
                with gr.Tabs() as main_input_tabs:
                    ...
                    with gr.Tab("🛠️ Dành cho Developer", id="dev_tab") as dev_tab:
                        ...
                        with gr.Accordion("🐍 1. Python SDK & Code mẫu", open=True):
                            ...
                            wav = tts.infer(
                                "Xin chào mọi người! [cười] Đây là giọng đọc từ VieNeu-TTS.", 
                                voice="Trúc Ly"
                            )
                            ...
                            sf.write("cloned_output.wav", wav_cloned, tts.sample_rate)
                            ```
                            """)
        ...
        def _voice_list_updates(select_voice=None):
            ...
            voice), SRT, Batch Studio, and the conversation speaker slots (keep their values)."""
            ...
            return [story, gr.update(choices=PRESET_VOICES_CACHE), gr.update(choices=PRESET_VOICES_CACHE), *([gr.update(choices=CONV_VOICES_CACHE)] * MAX_SPEAKERS)]

        def _save_voice(audio_path, name, desc, denoise):
            no_change = [gr.update()] * (3 + MAX_SPEAKERS)
            if not model_loaded or tts is None:
            ...
            msg = f"✅ Đã lưu giọng **{v_id}** — đã có trong danh sách giọng mẫu của Đọc truyện, Hội thoại, SRT và Batch Studio."
        ...
        btn_save_voice.click(
            ...
            outputs=[clone_save_status, user_voice_dd, voice_select, srt_voice, batch_voice, *speaker_voice_dds],
        )
        ...
        btn_generate_batch.click(lambda: gr.update(visible=False), outputs=[download_btn])
        btn_generate_batch.click(lambda: gr.update(interactive=True), outputs=btn_stop)
        batch_gen_event.then(lambda: gr.update(interactive=False), outputs=btn_stop)
        batch_gen_event.then(fn=on_audio_generated, inputs=[audio_output], outputs=[download_btn])

        # --- Audio Library & Viewer Handlers ---
        def _on_lib_refresh():
            proj_choices = list_projects()
            default_val = proj_choices[0][1] if proj_choices else None
            return gr.update(choices=proj_choices, value=default_val)

        def _on_lib_project_change(proj_name):
            if not proj_name:
                return [], "ℹ️ Chưa chọn dự án nào.", None, gr.update(visible=False), gr.update(visible=False)
            rows, summary, audio_p, merged_p = get_project_table_data(proj_name)
            merged_update = gr.update(value=merged_p, visible=bool(merged_p))
            zip_update = gr.update(visible=False)
            return rows, summary, audio_p, merged_update, zip_update

        library_tab.select(_on_lib_refresh, outputs=[lib_project_select])
        btn_lib_refresh.click(_on_lib_refresh, outputs=[lib_project_select])
        lib_project_select.change(
            _on_lib_project_change,
            inputs=[lib_project_select],
            outputs=[lib_dataframe, lib_summary_box, audio_output, lib_merged_download, lib_zip_download]
        )
        ...
        def _on_open_folder(proj_name):
            msg = open_project_folder(proj_name)
            return gr.update(value=msg, visible=True)

        btn_lib_open_folder.click(_on_open_folder, inputs=[lib_project_select], outputs=[lib_status_msg])

        def _on_export_zip(proj_name):
            zip_p, msg = export_project_zip(proj_name)
            return gr.update(value=zip_p, visible=bool(zip_p)), gr.update(value=msg, visible=True)

        btn_lib_export_zip.click(_on_export_zip, inputs=[lib_project_select], outputs=[lib_zip_download, lib_status_msg])

        def _on_lib_merge(proj_name, fmt, silence):
            if not proj_name:
                return gr.update(), gr.update(visible=False), gr.update(value="⚠️ Vui lòng chọn dự án cần nối.", visible=True), []
            merged_p, msg = merge_project_audio(PROJECTS_DIR / proj_name, output_format=fmt, silence_gap=float(silence))
            rows, summary, _, _ = get_project_table_data(proj_name)
            return merged_p, gr.update(value=merged_p, visible=bool(merged_p)), gr.update(value=msg, visible=True), rows

        btn_lib_merge.click(
            _on_lib_merge,
            inputs=[lib_project_select, lib_merge_format, lib_merge_silence],
            outputs=[audio_output, lib_merged_download, lib_merge_status, lib_dataframe]
        )
